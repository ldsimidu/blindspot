...
O core v0.10 inclui governança de Design System, Image System, arquitetura visual por tela, curadoria de movimento/UI externa, checkpoint de primeira renderização, loop de reabertura por feedback humano, avaliação de densidade/gravidade de estados, topografia estável de feedback, geometria de sinais visuais e validação progressiva. Tokens, componentes e direção visual concretos continuam pertencendo ao consumidor; o PEK padroniza a evidência, a precedência, as decisões, a composição revisável e as verificações.
