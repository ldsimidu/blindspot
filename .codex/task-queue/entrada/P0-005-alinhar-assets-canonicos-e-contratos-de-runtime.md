...
- Verificação bloqueada: `npm run build` falha por condição preexistente fora deste recorte: o script exige `vite.config.ts` ausente e o Vite tenta ler diretório sem permissão. A falha não foi mascarada nem corrigida nesta task.
